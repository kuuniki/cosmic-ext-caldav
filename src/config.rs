...
/// Returns the config directory, refusing to fall back to /tmp which is
/// world-readable.  Panicking here is acceptable — if the OS cannot provide
/// a home or config directory the application cannot function securely.
fn config_dir() -> PathBuf {
    dirs::config_dir()
        .or_else(|| dirs::home_dir().map(|h| h.join(".config")))
        .expect("Could not determine config directory — neither XDG_CONFIG_HOME nor HOME is set")
}
...
// Methods are shared between the settings app and the panel applet binaries;
// each binary uses a different subset, so unused warnings are expected.
#[allow(dead_code)]
impl Config {
    fn path() -> PathBuf {
